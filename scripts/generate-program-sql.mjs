...
const sourcePath = join(root, "lib/program/h2-v9-v1.json");
const outputPath = join(root, "migrations/010_seed_h2_v9_v1.sql");
...
function weightText(exercise) {
  ...
  if (percent && example) return `${percent} · при RMref 115: ${example}`;
...
function macrocycle(block, cycle) {
  if (block === "h2") return cycle <= 5 ? 1 : cycle <= 8 ? 2 : 3;
  return cycle <= 4 ? 1 : cycle <= 8 ? 2 : 3;
}

const lines = [
  "-- 010: Canonical H2→V9 v1.0 seed generated from lib/program/h2-v9-v1.json.",
  "-- Idempotent upserts by stable program keys. Legacy cycles and session history remain untouched.",
  "",
];
...
for (const [cycleIndex, cycle] of program.cycles.entries()) {
  ...
  lines.push(
    `INSERT INTO cycles (number, name, macrocycle, notes, sort_order, block, program_version, program_key, day_offset, checkpoint, rules) VALUES (` +
      `${cycle.number}, ${q(cycle.name)}, ${macrocycle(cycle.block, cycle.number)}, ${q(cycleNotes)}, ${cycleIndex + 1}, ${q(cycle.block)}, ${q(program.version)}, ${q(cycle.id)}, ${cycle.dayOffset}, ${cycle.checkpoint ? j(cycle.checkpoint) : "NULL"}, ${j(cycle.rules ?? [])}) ` +
  ...
  for (const [workoutIndex, workout] of cycle.workouts.entries()) {
    ...
    lines.push(
      `INSERT INTO workouts (cycle_id, label, title, notes, sort_order, kind, cardio_zone, cardio_minutes, program_key, day_in_cycle, warmup_level, prescription, branches) VALUES (` +
        `(SELECT id FROM cycles WHERE program_version=${q(program.version)} AND program_key=${q(cycle.id)}), ${q(workout.slot)}, ${q(workout.title)}, ${q((workout.notes ?? []).join("\n"))}, ${workoutIndex + 1}, ${q(workout.kind)}, ${q(workout.cardio?.zone ?? null)}, ${q(cardioText)}, ${q(workout.id)}, ${workout.day}, ${q(workout.warmupLevel)}, ${j(workout)}, ${j(workout.branches ?? [])}) ` +
    ...
    for (const [exerciseIndex, exercise] of seededExercises.entries()) {
      ...
      lines.push(
        `INSERT INTO workout_exercises (workout_id, sort_order, name, weight_text, pct_of_tm, target_reps, target_sets, target_rir_min, target_rir_max, comment, rest_seconds, program_key, role, pct_min, pct_max, example_kg_min, example_kg_max, target_rpe_min, target_rpe_max, is_optional, condition_code, exclude_from_tonnage, prescription) VALUES (` +
          `(SELECT id FROM workouts WHERE program_key=${q(workout.id)}), ${exerciseIndex + 1}, ${q(exercise.name)}, ${q(weightText(exercise))}, ${exercise.percent?.min === exercise.percent?.max ? n(exercise.percent?.min) : "NULL"}, ${q(exercise.reps)}, ${q(exercise.sets)}, ${n(targetRirMin)}, ${n(targetRirMax)}, ${q(comments)}, ${restSeconds(exercise)}, ${q(exercise.id)}, ${q(exercise.role)}, ${n(exercise.percent?.min)}, ${n(exercise.percent?.max)}, ${n(exercise.exampleKg?.min)}, ${n(exercise.exampleKg?.max)}, ${n(exercise.targetRpe?.min)}, ${n(exercise.targetRpe?.max)}, ${exercise.optional ? "true" : "false"}, ${q(exercise.condition)}, ${exercise.excludeFromTonnage ? "true" : "false"}, ${j(exercise)}) ` +
          `ON CONFLICT (program_key) WHERE program_key IS NOT NULL DO UPDATE SET ` +
