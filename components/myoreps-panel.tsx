...
export function MyorepsPanel({
  ...
  const allowed = isMyorepsAllowed(block, cycleNumber)
  ...
  const stopReason = useMemo(
    () => myorepsStopReason({ miniSets, lastMiniSetReps: lastReps, techniqueOk, pain }),
        ...
        <div className="mt-3 space-y-3 text-sm">
          {!allowed ? (
            <p className="rounded-md bg-secondary p-2 text-xs leading-relaxed">
              В AMRAP-циклах, разгрузке, пике и тесте миорепсы отключены ради свежести.
            </p>
          ) : (
            <>
              {!activated ? (
                <>
                  <p className="text-muted-foreground">
                    Выполни обычный активирующий подход до RIR 1–2. Не доводи его до отказа.
                  </p>
                  <button
                    type="button"
                    className="w-full rounded-lg bg-primary px-3 py-3 font-semibold text-primary-foreground"
                    onClick={() => { setActivated(true); beginRest() }}
                  >
                ...
                <>
                  <div className="flex items-center justify-between rounded-md bg-background px-3 py-2">
                    ...
                    <strong>{miniSets} / {MYOREPS_MAX_MINI_SETS}</strong>
                  </div>
                  ...
                  {endAt ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-muted-foreground">
                        Выполни {MYOREPS_MIN_REPS}–{MYOREPS_MAX_REPS} чистых повторов и запиши их обычной кнопкой подхода.
                      </p>
                      <label className="block text-xs font-medium text-muted-foreground" htmlFor="myoreps-reps">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        value={lastReps ?? ""}
                        onChange={(event) => setLastReps(event.target.value ? Number(event.target.value) : null)}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <label className="flex items-center gap-2 rounded-md border border-border p-2">
                          <input type="checkbox" checked={techniqueOk} onChange={(event) => setTechniqueOk(event.target.checked)} />
                          Техника чистая
                        </label>
                        <label className="flex items-center gap-2 rounded-md border border-border p-2">
                      ...
                      <button
                        ...
                        disabled={lastReps == null || lastReps < MYOREPS_MIN_REPS || !techniqueOk || pain}
                        onClick={() => {
              ...
              <button type="button" className="w-full text-xs text-muted-foreground" onClick={reset}>
                Сбросить протокол
              </button>
              <p className="text-xs leading-relaxed text-muted-foreground">
                Стоп раньше лимита при боли, раскачке, резком падении скорости или если получилось меньше 3 повторов.
              </p>
            </>
          )}
        </div>
