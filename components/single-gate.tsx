...
const REASON_TEXT: Record<string, string> = {
  ...
  "medical-clearance-required": "нет явного допуска к планируемому натуживанию",
  ...
  "expected-rpe-outside-6-to-7": "ожидаемый RPE вне цели 6–7",
};
  ...
  async function check() {
    ...
    try {
      const decision = await evaluateConditionalSingle({
        sessionId,
        medicalClearanceForPlannedLoadAndStraining: clearance,
  ...
  return (
    <section className="rounded-xl border border-primary/40 bg-primary/5 p-4">
      ...
      <div className="mt-3 space-y-1">
        <Check label="Есть явный персональный допуск к планируемой нагрузке и натуживанию" value={clearance} set={setClearance} />
      ...
      <button type="button" disabled={busy} onClick={() => void check()} className="mt-3 min-h-11 w-full rounded-lg bg-primary px-4 font-semibold text-primary-foreground disabled:opacity-50">
        {busy ? "Проверяю…" : "Проверить все условия"}
      </button>
      {result && (
        <div className={`mt-3 rounded-lg p-3 text-sm ${result.allowed ? "bg-success/15 text-success" : "bg-destructive/10 text-destructive"}`}>
          {result.allowed ? (
            <p><strong>Разрешено:</strong> {result.plannedSingles === 3 ? "репетиция 3×1" : "один запланированный сингл"}; цель RPE 6–7, потолок 8.</p>
          ) : (
