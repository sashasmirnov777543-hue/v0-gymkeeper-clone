...
import {
  detectCoachRedFlags,
  redFlagCoachResponse,
} from "../lib/coach/safety.ts";
...
test("local safety gate catches high-signal red-flag language before the model", () => {
  assert.deepEqual(
    detectCoachRedFlags(
      "У меня внезапно появилась тень как занавес в поле зрения",
    ),
    ["vision"],
  );
  assert.deepEqual(
    detectCoachRedFlags("Во время жима появилась боль и давление в груди"),
    ["chest"],
  );
  assert.deepEqual(
    detectCoachRedFlags("Нарастающее онемение ноги и не могу начать мочиться"),
    ["neurologic"],
  );
...
test("ordinary training language does not trigger the local emergency gate", () => {
  assert.deepEqual(
    detectCoachRedFlags(
      "Сегодня плохо спал, разминка была тяжёлая, что уменьшить?",
    ),
    [],
  );
});

test("coach prompt preserves revision-3 invariants and explicit confirmation", () => {
  assert.match(COACH_SYSTEM_PROMPT, /не меняешь журнал\/план напрямую/);
  assert.match(COACH_SYSTEM_PROMPT, /после явного подтверждения/);
  assert.match(COACH_SYSTEM_PROMPT, /Ц17A\/Ц19A\/Ц21A/);
  assert.match(COACH_SYSTEM_PROMPT, /Красный: никакой силовой, кардио/);
  assert.match(COACH_SYSTEM_PROMPT, /Не добавлять AMRAP/);
  assert.match(COACH_SYSTEM_PROMPT, /Общая надбавка Δ отменена/);
});
...
test("proposal validation fails closed on unsafe or over-broad patches", () => {
  ...
  assert.ok(
    unsafe.errors.includes("unsupported-patch-key:increaseWeightPercent"),
  );
  assert.ok(
    unsafe.errors.includes("red-readiness-cannot-create-training-change"),
  );
});
