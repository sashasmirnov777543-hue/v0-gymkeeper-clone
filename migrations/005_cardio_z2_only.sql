...
UPDATE workouts w SET
  ...
  notes = CASE
    ...
    WHEN r.rn = 1 AND r.cycle_number IN (3, 4) THEN 'Только Z2 — разговорный темп (полными предложениями говорить можно, петь нет). Велосипед или эллипс. Перед тяжёлым днём А — потолок 60 мин. Плохой сон или пульс покоя +5 уд/мин → срезать до 30 мин или заменить прогулкой. Опционально 1 раз за цикл: финишер 4–5 мин Z3 (комфортно тяжело) в конце — только при хорошем самочувствии. Разминка/заминка: первые 5 мин — плавный разгон из Z1, последние 5 мин — сброс в Z1 (входят в указанные минуты). С тренажёра сходить и вставать медленно — ортостатика.'
    WHEN r.rn = 1 THEN 'Только Z2 — разговорный темп (полными предложениями говорить можно, петь нет). Велосипед или эллипс, без интервалов. Перед тяжёлым днём А — потолок 60 мин. Плохой сон или пульс покоя +5 уд/мин → срезать до 30 мин или заменить прогулкой. Разминка/заминка: первые 5 мин — плавный разгон из Z1, последние 5 мин — сброс в Z1 (входят в указанные минуты). С тренажёра сходить и вставать медленно — ортостатика.'
    ELSE 'Z2 восстановительная: разговорный темп, без интервалов. Первые 3 мин — мягкий разгон, последние 3 мин — заминка в Z1. По самочувствию можно заменить прогулкой. Вечером хорошо для сна.'
  END
FROM ranked r
WHERE w.id = r.id;
