...
function eventToResult(row: typeof tmRecalcEvents.$inferSelect): TmRecalcResult {
  return {
...
async function updateMacroExercises(
  ...
  const exercises = await tx
    .select({ id: workoutExercises.id, weightText: workoutExercises.weightText })
...
export async function applyAmrapTmRecalc(
  ...
  return db.transaction(async (tx) => {
    ...
    const amrapSets = await tx
      ...
      .where(and(eq(loggedSets.sessionId, sessionId), ilike(workoutExercises.name, "%AMRAP%")))

    let best: { weight: number; reps: number; e1rm: number; tm: number } | null = null
    for (const set of amrapSets) {
      const weight = set.weight == null ? null : Number.parseFloat(set.weight)
      if (weight == null || weight <= 0 || set.reps == null || set.reps <= 0) continue
      const calc = trainingMaxFromAmrap(weight, set.reps)
      if (!best || calc.e1rm > best.e1rm) best = { weight, reps: set.reps, ...calc }
    }
    if (!best) return null
    ...
    const [oldRow] = await tx.select().from(appSettings).where(eq(appSettings.key, key)).limit(1)
    const oldTm = oldRow ? Number.parseFloat(oldRow.value) : null
    await tx.insert(appSettings).values({ key, value: String(newTm) }).onConflictDoUpdate({
...
export async function applyManualTrainingMax(macro: number, newTm: number): Promise<number> {
  await ensureSchema()
...
export async function latestAmrapSessionId(): Promise<number | null> {
  ...
  const [row] = await db
    ...
    .innerJoin(workoutExercises, eq(loggedSets.workoutExerciseId, workoutExercises.id))
    .where(and(eq(sessions.status, "completed"), ilike(workoutExercises.name, "%AMRAP%")))
    .orderBy(desc(sessions.finishedAt))
    .limit(1)
  return row?.id ?? null
}

export async function undoLatestTmRecalc(): Promise<{ macro: number; restoredTm: number } | null> {
  await ensureSchema()
  const [event] = await db.select().from(tmRecalcEvents).orderBy(desc(tmRecalcEvents.createdAt)).limit(1)
  ...
  await db.delete(tmRecalcEvents).where(eq(tmRecalcEvents.sessionId, event.sessionId))
