...
export default async function HomePage() {
  const [
    ...
    db
      ...
      .orderBy(asc(recoveryInsertions.afterProgramDay)),
    db
      .select()
      .from(cycles)
  ...
  const programWorkouts = allWorkouts.filter((workout) =>
    cycleIds.has(workout.cycleId),
  );
  const workoutIds = new Set(programWorkouts.map((workout) => workout.id));
  const programSessions = sessionRows.filter((session) =>
    workoutIds.has(session.workoutId),
  );
  ...
  const activeSession =
    sessionRows.find((session) => session.status === "active") ?? null;
  ...
  const currentCycle = allCycles.find(
    (cycle) =>
      cycle.block === descriptor.block && cycle.number === descriptor.cycle,
  );
  ...
  const next =
    orderedWorkouts.find(
      ({ workout, programDay }) =>
        ...
        !(workout.prescription as { isRestDay?: boolean } | null)?.isRestDay &&
    ...
    return (
      <main className="mx-auto min-h-dvh w-full max-w-lg px-4 pb-40 pt-8">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Настроить календарь программы
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          Выберите дату P1 первого цикла H2. Приложение построит все 22
          восьмидневных цикла; дополнительные дни восстановления будут сдвигать
          календарь, а не сжимать его.
        </p>
        <form
          action={setProgramStartDate}
        ...
        >
          <label className="text-sm font-medium">
            ...
            <input
              ...
              required
              defaultValue={today}
              className="mt-2 h-12 w-full rounded-lg border border-input bg-background px-3 text-base"
            />
          </label>
          <button className="mt-4 min-h-12 w-full rounded-lg bg-primary px-4 font-bold text-primary-foreground">
            Построить 176-дневный календарь
          </button>
        </form>
        <BottomNav />
      </main>
  ...
  return (
    <div className="min-h-screen bg-background pb-40">
      <header className="border-b border-border bg-card">
        <div className="mx-auto max-w-lg px-4 py-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="font-mono text-xs uppercase tracking-widest text-primary">
                {descriptor.block.toUpperCase()} · цикл {descriptor.cycle}/{" "}
                {descriptor.block === "h2" ? 9 : 13}
              </p>
              <h1 className="mt-1 text-2xl font-bold tracking-tight">
                День {currentProgramDay} из 176
              </h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {currentCycle?.name ?? "Программа H2→V9"}
              </p>
            </div>
          ...
          <div
            ...
            role="progressbar"
            aria-valuenow={currentProgramDay}
            aria-valuemin={1}
            aria-valuemax={176}
          >
            <div
          ...
          <div className="mt-1 flex justify-between text-xs text-muted-foreground">
            <span>{progressPercent}% цикла</span>
            <span>
              {completedWorkoutIds.size} завершённых сессий · 44 зала по плану
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-4 py-4">
        <Link
          href="/program-guide"
          className="flex min-h-12 items-center justify-between rounded-xl border border-border bg-card px-4 text-sm font-semibold"
        >
          Редакция 3.0 · правила и матрица <ArrowRight className="size-4" />
        </Link>
        {activeSession && (
          <Link
            href={`/session/${activeSession.id}`}
            className="flex min-h-14 items-center justify-between rounded-xl bg-primary px-4 font-semibold text-primary-foreground"
          >
            <span>Продолжить активную тренировку</span>
        ...
        <section className="rounded-2xl border border-border bg-card p-4">
          <div className="flex items-start gap-3">
            <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-primary/15 text-primary">
              ...
              ) : descriptor.slot === "B1" || descriptor.slot === "B3" ? (
            ...
            <div className="min-w-0 flex-1">
              ...
              {todayItem?.kind === "recovery" ? (
                <>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {todayItem.label ??
                      "Сон, питание и обычная активность. Ничего не догонять."}
                  </p>
                </>
              ) : currentProgramDay === 171 ? (
                <>
                  <h2 className="mt-1 text-xl font-bold">
                    Полный тренировочный отдых
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    День 171: 0 минут кардио, 0 тренировочных подходов. Не
                    добавлять утомление перед итогом.
                  </p>
                </>
              ) : descriptor.slot.startsWith("P") ? (
                <>
                  <h2 className="mt-1 text-xl font-bold">
                    {descriptor.slot} · рабочая смена
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Силовой работы нет. Сон и восстановление важнее шагов и
                    необязательной активности.
                  </p>
                </>
              ) : next ? (
                <>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {next.cycle.block.toUpperCase()}-{next.cycle.number} ·{" "}
                    {next.workout.label} · день {next.programDay}
                  </p>
                </>
              ) : (
          ...
          */}
          {!activeSession && next && currentProgramDay !== 171 && (
            <Link
        ...
        <section className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="mt-4 grid grid-cols-8 gap-1.5">
            {CYCLE_SLOTS.map((slot, index) => {
              const workout = currentCycleWorkouts.find(
                (item) => item.label === slot,
              );
              const done = workout
                ? completedWorkoutIds.has(workout.id)
                : false;
              ...
              return (
                <Link
                  ...
                  className={style}
                  title={`День ${index + 1} · открыть план`}
                >
                  {body}
                </Link>
              );
        ...
        <section className="grid grid-cols-2 gap-3">
          <div
            className={`rounded-2xl border p-4 ${rhr.yellowSignal ? "border-warning/50 bg-warning/10" : "border-border bg-card"}`}
          >
            <div className="flex items-center justify-between">
              <HeartPulse className="size-5 text-primary" />
              <span className="text-xs text-muted-foreground">RHR</span>
            </div>
            ...
            {rhr.yellowSignal && (
              <p className="mt-2 text-xs font-medium text-warning">
                Минимум жёлтый вместе с плохим самочувствием.
              </p>
            )}
          </div>
          <div className="rounded-2xl border border-border bg-card p-4">
            ...
            <p className="mt-3 text-sm font-semibold">
              Светофор перед каждой сессией
            </p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              Редакция 3.0: фактическое усилие, личные ограничения и страховка.
              КТ и 1ПМ не обязательны.
            </p>
          </div>
        </section>
        ...
        <details className="rounded-2xl border border-border bg-card p-4">
          <summary className="cursor-pointer font-semibold">
            Записать утренний RHR
          </summary>
          <form
            action={saveRhrMeasurement}
          ...
          >
            ...
            <div className="space-y-1 pt-4 text-sm">
              <label className="flex gap-2">
                <input name="comparable" type="checkbox" defaultChecked />{" "}
                Сопоставимое утро
              </label>
              <label className="flex gap-2">
                <input name="poorWellbeing" type="checkbox" /> Плохое
                самочувствие
              </label>
            </div>
            ...
            <button className="col-span-2 min-h-11 rounded-lg bg-primary px-4 font-semibold text-primary-foreground">
              Сохранить RHR
            </button>
          </form>
        </details>
        ...
        <section className="rounded-2xl border border-border bg-card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold">Gemini 3.5 Flash всегда рядом</h2>
              <p className="text-xs text-muted-foreground">
                Чат закреплён над навигацией.{" "}
                {pending.length > 0
                  ? `Ожидают подтверждения: ${pending.length}.`
                  : "Корректировки без вашего подтверждения не применяются."}
              </p>
            </div>
          </div>
        </section>
